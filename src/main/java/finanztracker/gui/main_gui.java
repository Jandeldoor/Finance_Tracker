...
public class main_gui {
    ...
    private boolean rentFilter = false, foodFilter = false, transportFilter = false, leisureFilter = false, wageFilter = false;
    ...
    private void createFilterBoxes(Pane pane) {
        ...
        FontIcon wageIcon = new FontIcon(FontAwesomeSolid.WALLET);
        wageIcon.setIconSize(20);
        wageIcon.setFill(Color.GRAY);
        CheckBox wageCheckbox = new CheckBox("Gehalt");
        wageIcon.setLayoutX(20);
        wageIcon.setLayoutY(460);
        wageCheckbox.setLayoutX(45);
        wageCheckbox.setLayoutY(445);
        wageCheckbox.setTextFill(Color.WHITE);
        wageCheckbox.getStyleClass().add("check-box");



        pane.getChildren().addAll(rentIcon, rentCheckBox, foodIcon, foodCheckBox, transportIcon, transportCheckbox, leisureIcon, leisureCheckbox, wageIcon, wageCheckbox);
        ...
        addFilterHoverEffect(wageIcon, "Filter: Gehalt");
        ...
        wageCheckbox.setOnAction(event -> wageFilter = !wageFilter);
    }
    ...
    private void applyFilters() {
        ...
        if (wageFilter) {
            if (firstCategory) {
                incomeQuery.append(" AND e.kategorie = 'Gehalt'");
                firstCategory = false;
            } else {
                incomeQuery.append(" OR e.kategorie = 'Gehalt'");
            }
        }
        ...
        if (wageFilter) {
            if (firstCategory) {
                expenseQuery.append(" AND a.kategorie = 'Gehalt'");
                firstCategory = false;
            } else {
                expenseQuery.append(" OR a.kategorie = 'Gehalt'");
            }
        }
